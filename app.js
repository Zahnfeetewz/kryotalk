...
window.onerror = function(msg, src, line, col, err) {
  ...
  if (!el) { el = document.createElement('div'); el.id = 'js-error-banner'; el.style.cssText = 'position:fixed;top:0;left:0;right:0;z-index:99999;background:#f23f43;color:#fff;padding:12px 20px;font-size:13px;font-family:monospace;word-break:break-all;cursor:pointer'; document.body.appendChild(el); }
...
if (!window.io) window.io = function() { return { on: function(){}, emit: function(){}, disconnect: function(){} }; };
...
document.getElementById('app-verify-submit')?.addEventListener('click', async () => {
  ...
  try {
    ...
    btn.disabled = false; btn.textContent = t('verifyEmail');
    if (!result || !result.ok) { errorEl.textContent = (result && result.error) || t('error'); return; }
...
// duplicate keydown handler removed - original at line 436
...
function applyProfileToUI() {
  ...
  if (statusText) statusText.textContent = statusLabels[curStatus] || '';
...
function showStatusSelector() {
  ...
  setTimeout(() => {
    const handler = (e) => {
      if (!popup.contains(e.target) && e.target.id !== 'user-status-dot' && !e.target.closest('#user-status-dot')) {
        popup.classList.add('hidden');
...
async function loadServers() {
  var _srvRes = await window.api.serverGetForUser(currentUser.id); allServers = (_srvRes && _srvRes.servers) || [];
  ...
  menu.querySelectorAll('.context-menu-item').forEach((item) => {
    item.addEventListener('click', async () => {
      ...
      if (action === 'copy') {
        ...
        const emoji = prompt(t('enterEmoji'));
        if (emoji) {
          await window.api.messagesReact(msgId, emoji, userId);
...
function startEditMessage(msgId, currentContent) {
  const contentEl = document.getElementById('msg-content-' + msgId);
  if (!contentEl) return;
  contentEl.innerHTML = '<input class="msg-edit-input" type="text" value="" />';
  ...
  const save = async () => {
    ...
    if (!newContent || newContent === currentContent) { contentEl.textContent = currentContent; return; }
    try { if (window.api.messagesEdit) await window.api.messagesEdit(msgId, newContent); } catch {}
    contentEl.textContent = newContent;
  };
  input.addEventListener('keydown', (ev) => {
    if (ev.key === 'Enter' && !ev.shiftKey) { ev.preventDefault(); save(); }
    if (ev.key === 'Escape') { contentEl.textContent = currentContent; }
  });
...
async function loadDMList() {
  ...
  for (const dm of dms) {
    ...
    if (lastMsg && lastMsg.authorId !== userId && lastMsg.timestamp > lastSeen) {
      ...
      if (!unreadDMs[dm.id]) unreadDMs[dm.id] = false;
    }
...
async function openDM(dm) {
  ...
  document.getElementById('server-settings-btn')?.classList.add('hidden');
  document.getElementById('toggle-members')?.classList.add('hidden');
  var _dma = document.getElementById('dm-call-actions'); if (_dma) _dma.style.display = 'flex';
  hideMembersSidebar();
  const otherId = dm.participants?.find((p) => p !== userId);
...
function hideDMCallButtons() {
  var _dma = document.getElementById('dm-call-actions'); if (_dma) _dma.style.display = 'none';
}
...
async function showMembersSidebar() {
  ...
  var resolvedMembers = (membersRes?.users || []).map(function(u) { return u || null; }).filter(Boolean);
...
document.getElementById('admin-login-btn')?.addEventListener('click', async () => {
  ...
  const result = await window.api.adminLogin(pw, currentUser?.username);
  if (!result?.ok) {
    if (result?.firstTime) {
...
async function loadAdminUsers() {
  const res = await window.api.adminGetAllUsers();
...
function showSaveBar() { var bar = document.getElementById('settings-save-bar'); if (bar) { bar.style.display = 'flex'; } }
...
function buildSettingsNav() {
  ...
  sections.push({ id: 'admin', label: '🛡️ ' + t('navAdminPanel') });
  ...
  nav.querySelectorAll('.settings-nav-item').forEach(function(item) {
    item.addEventListener('click', function() {
      if (_hasPendingChanges) { if (!confirm(t('unsavedChanges'))) return; }
      hideSaveBar();
...
function renderSettingsSection(id) {
  ...
  if (fn) try { fn(content); } catch(e) { console.error('[Settings] Error rendering '+id+':', e); content.innerHTML = '<div class="settings-card"><div class="settings-label">Fehler beim Laden</div><div style="font-size:13px;color:var(--text-muted)">'+escHtml(e.message)+'</div></div>'; }
}
...
settingsSections.account = function(el) {
  ...
  var servers = (Array.isArray(allServers) ? allServers : []).filter(s => s.members && s.members.some(m => (m.id || m.userId) === userId));
  ...
  document.getElementById('s-2fa-setup-btn')?.addEventListener('click', async function() {
    ...
    area.innerHTML =
      ...
      '<div style="text-align:center;margin-bottom:16px">' +
        '<img src="' + (res.qrCode || res.dataUrl || '') + '" style="width:220px;height:220px;border-radius:12px;background:#fff;padding:12px" />' +
      '</div>' +
      '<div style="text-align:center;margin-bottom:16px">' +
...
settingsSections.admin = function(el) {
  if (localStorage.getItem('adminAuth')) {
    var ghToken = localStorage.getItem('kryotalk_gh_token') || '';
    el.innerHTML = '<div class="settings-section-title">' + t('navAdminPanel') + '</div><div id="settings-admin-container"></div>' +
      ...
      '<div style="font-size:13px;font-weight:600;margin-bottom:8px">GitHub Token (fuer Usernamen-Registry)</div>' +
      '<input type="password" class="settings-input" id="settings-gh-token" placeholder="ghp_..." value="' + ghToken.replace(/"/g, '&quot;') + '" style="width:100%" />' +
      '<button class="settings-btn settings-btn-primary" id="settings-gh-token-save" style="margin-top:8px">Token speichern</button>' +
      '</div>';
    document.getElementById('settings-gh-token-save')?.addEventListener('click', function() {
      var v = document.getElementById('settings-gh-token')?.value?.trim() || '';
      if (v) localStorage.setItem('kryotalk_gh_token', v);
      else localStorage.removeItem('kryotalk_gh_token');
    });
    ...
    document.getElementById('settings-admin-login-btn')?.addEventListener('click', async function() {
      ...
      var result = await window.api.adminLogin(pw, currentUser?.username);
...
async function loadAdminUsersInto(container) {
  if (!container) return;
  var res = await window.api.adminGetAllUsers();
...
// status click handled by #user-status-dot listener above
...
// ── Init ──
var _initChain = (window.api?.ready || Promise.resolve()).then(function() { return loadProfile(); });
_initChain.then(async () => {
...
if (mobileMenuBtn) {
  mobileMenuBtn.addEventListener('click', function(e) {
    e.stopPropagation();
    if (layoutEl) layoutEl.classList.toggle('menu-open');
  });
}
...
document.querySelector('.chat-area')?.addEventListener('click', function() {
  if (layoutEl && layoutEl.classList.contains('menu-open')) {
